...
task(
    'namefi-set-nfsc-address',
    'Call NamefiNFT.setServiceCreditContract(NamefiServiceCredit.address)'
)
    .addParam(
        'nft',
        'The address to NamefiNFT',
        '0x0000000000cf80E7Cf8Fa4480907f692177f8e06'
    )
    .addParam(
        'nfsc',
        'The address to NamefiServiceCredit',
    ...
    .setAction(async function (taskArguments: TaskArguments, {ethers, run}) {
        ...
        const nfscContract = await ethers.getContractAt(
            'NamefiServiceCredit',
            nft
        )
...
task('namefi-grant-nft-minter', 'Call NamefiNFT.grantRole(MINTER_ROLE, minter)')
    ...
    .setAction(async function (taskArguments: TaskArguments, {ethers, run}) {
        ...
        if (taskArguments.minter) {
            ...
            const tx = await nftContract.grantRole(
                MINTER_ROLE,
                taskArguments.minter
            )
            await gasReport(tx, ethers.provider)
        } else {
...
task(
    'namefi-grant-nfsc-minter',
    'Call NamefiServiceCredit.grantRole(MINTER_ROLE, minter)'
)
    .addParam(
        'nfsc',
        'The address to NamefiServiceCredit',
    ...
    .setAction(async function (taskArguments: TaskArguments, {ethers, run}) {
        ...
        if (taskArguments.minter) {
            ...
            const tx = await nfscContract.grantRole(
                MINTER_ROLE,
                taskArguments.minter
            )
            await gasReport(tx, ethers.provider)
...
task(
    'namefi-grant-admins',
    'Call NamefiServiceCredit.grantRole(DEFAULT_ADMIN_ROLE, admin) and NamefiNFT.grantRole(DEFAULT_ADMIN_ROLE, admin)'
)
    .addParam(
        'nft',
        'The address to NamefiNFT',
    ...
    .setAction(async function (taskArguments: TaskArguments, {ethers, run}) {
        ...
        if (taskArguments.admin) {
            ...
            console.assert(
                DEFAULT_ADMIN_ROLE === (await nftContract.DEFAULT_ADMIN_ROLE()),
                'DEFAULT_ADMIN_ROLE mismatch'
            )
            const tx = await nftContract.grantRole(
                DEFAULT_ADMIN_ROLE,
                taskArguments.admin
            ...
            await gasReport(tx, ethers.provider)
            const tx2 = await nfscContract.grantRole(
                DEFAULT_ADMIN_ROLE,
                taskArguments.admin
            )
            await gasReport(tx2, ethers.provider)
...
task(
    'namefi-proxy-admin-transfer-owner',
    'Call NamefiServiceCredit.grantRole(DEFAULT_ADMIN_ROLE, admin) and NamefiNFT.grantRole(DEFAULT_ADMIN_ROLE, admin)'
)
    ...
    .setAction(async function (taskArguments: TaskArguments, {ethers, run}) {
        ...
        if (taskArguments.newOwner) {
            const tx = await proxyAdminContract.transferOwnership(
                taskArguments.newOwner
            )
            await gasReport(tx, ethers.provider)
        } else {
