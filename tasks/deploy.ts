...
task('print-signers', 'Printer signers').setAction(async function (
...
task('namefi-logic-deploy', 'Deploy the logic contract')
    ...
    .setAction(async function (taskArguments: TaskArguments, {ethers, run}) {
        ...
        console.log(
            `Contract ${taskArguments.logicContractName} deployed to ${contract.address}`
        )
        ...
        console.log(
            `Contract ${taskArguments.logicContractName} verified at ${contract.address}.`
        )
    })
...
task('namefi-admin-deploy', 'Deploy the ProxyAdmin contract').setAction(
    async function (taskArguments: TaskArguments, {ethers, run}) {
        const signers = await ethers.getSigners()
        const signer = signers[0]
...
task('namefi-upgrade', 'Deploy the ProxyAdmin contract')
    ...
    .setAction(async function (taskArguments: TaskArguments, {ethers, run}) {
        ...
        const signer = signers[0]
        const slotIndex =
            '0xb53127684a568b3173ae13b9f8a6016e243e63b6e8ee1178d6a717850b5d6103' // AdminAddress https://eips.ethereum.org/EIPS/eip-1967
        ...
        const adminAddress = ethers.utils.getAddress(
            '0x' + adminAddressBytes32.slice(26)
        )
        ...
        console.log(
            `Done waiting for the confirmation for contract upgrade at ${taskArguments.proxyAddress}`
        )
    })
