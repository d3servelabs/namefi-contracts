...
describe("D3BridgeNFT", function () {
  ...
  it("Test contract", async function () {
    ...
    await expect(instance.connect(alice).safeMintByName(bob.address, normalizedDomainName))
      .to.be.revertedWith("Ownable: caller is not the owner");
    const tx = await instance.connect(deterministicTestSigner).safeMintByName(bob.address, normalizedDomainName);
