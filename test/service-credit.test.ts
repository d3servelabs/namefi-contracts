...
describe('NamefiServiceCredit', function () {
    ...
    async function deployFixture() {
        ...
        const {instance: nftInstance, logic: nftLogic} =
            await creatUpgradableContract('NamefiNFT', proxyAdmin)
        await nftInstance
            .connect(contractDeploySigner)
            .grantRole(DEFAULT_ADMIN_ROLE, nftDefaultAdmin.address)
        await nftInstance
            .connect(contractDeploySigner)
        ...
        await nftInstance
            .connect(contractDeploySigner)
            .renounceRole(PAUSER_ROLE, contractDeploySigner.address)

        const {instance: scInstance} = await creatUpgradableContract(
            'NamefiServiceCredit',
            proxyAdmin
        )
        await scInstance
            .connect(contractDeploySigner)
            .grantRole(MINTER_ROLE, scMinter.address)
        await scInstance
            .connect(contractDeploySigner)
            .grantRole(DEFAULT_ADMIN_ROLE, scDefaultAdmin.address)
        expect(
            await scInstance.hasRole(DEFAULT_ADMIN_ROLE, scDefaultAdmin.address)
        ).to.be.true
        await scInstance
            .connect(contractDeploySigner)
            .renounceRole(DEFAULT_ADMIN_ROLE, contractDeploySigner.address)
        await scInstance
            ...
            .renounceRole(MINTER_ROLE, contractDeploySigner.address)
        expect(
            await scInstance.hasRole(
                DEFAULT_ADMIN_ROLE,
                contractDeploySigner.address
            )
        ).to.be.false
        await scInstance
            .connect(scDefaultAdmin)
            .grantRole(PAUSER_ROLE, scPauser.address)
    ...
    it('should be able to be charged to mint a new name', async function () {
        ...
        await scInstance
            .connect(scMinter)
            .mint(alice.address, ethers.utils.parseUnits('100', 18))
        await scInstance
            .connect(scDefaultAdmin)
            .revokeRole(MINTER_ROLE, scMinter.address)
        ...
        expect(
            await scInstance.connect(scMinter).balanceOf(alice.address)
        ).to.equal(ethers.utils.parseUnits('100', 18))
        await scInstance
            .connect(scDefaultAdmin)
            .grantRole(CHARGER_ROLE, nftInstance.address)
        await nftInstance
            .connect(nftDefaultAdmin)
            .grantRole(MINTER_ROLE, nftMinter.address)
        await nftInstance
            .connect(nftDefaultAdmin)
            .setServiceCreditContract(scInstance.address)
        const tx = await nftInstance
            .connect(nftMinter)
        ...
        expect(
            await nftInstance.ownerOf(ethers.utils.id(normalizedDomainName))
        ).to.equal(bob.address)
        expect(
            await nftInstance.ownerOf(ethers.utils.id('bob.alice.eth'))
        ).to.equal(bob.address)
        expect(await scInstance.balanceOf(alice.address)).to.equal(
    ...
    it('DEPRECATED should be able to be charged default amount to mint extend a domain with a multiple of year', async function () {
        ...
        const tx2 = await nftInstance.connect(nftMinter).extendByNameWithCharge(
            normalizedDomainName,
        ...
        expect(
            await nftInstance.getExpiration(
                ethers.utils.id(normalizedDomainName)
            )
        ).to.equal(expirationTime + 60 * 60 * 24 * 365 * 2)
    })
    ...
    it('should be able to be buyable with ethers', async function () {
        ...
        await expect(scInstance.price(ethers.constants.AddressZero))
            ...
            .to.be.revertedWithCustomError(
                scInstance,
                'NamefiServiceCredit_UnsupportedPayToken'
            )
        ...
        expect(await scInstance.price(ethers.constants.AddressZero)).to.equal(
            ethers.utils.parseUnits('2', 6)
        )
        ...
        expect(
            scInstance
                .connect(alice)
                .buyWithEthers({value: ethers.utils.parseUnits('1', 18)})
        )
            // .to.be.revertedWith("NamefiServiceCredit: insufficient purchasble supply");
            .to.be.revertedWithCustomError(
                scInstance,
                'NamefiServiceCredit_InsufficientBuyableSupply'
            )

        let tx = await scInstance
            .connect(scMinter)
            .increaseBuyableSupply(ethers.utils.parseUnits('1000', 18))
        let rc = await tx.wait()
        let event = rc.events?.find(
            (e: any) => e.event === 'IncreaseBuyableSupply'
        )
        expect(event).to.not.be.undefined
        expect(event?.args?.increaseBy).to.equal(
            ethers.utils.parseUnits('1000', 18)
        )
        expect(await scInstance.connect(scMinter).buyableSupply()).to.equal(
            ethers.utils.parseUnits('1000', 18)
        )

        let tx2 = await scInstance
            .connect(alice)
            .buyWithEthers({value: ethers.utils.parseUnits('0.5', 18)})
        // check that tx2 contains the event BuyToken
        rc = await tx2.wait()
        event = rc.events?.find((e: any) => e.event === 'BuyToken')
        expect(event).to.not.be.undefined
        expect(event?.args?.buyAmount).to.equal(
        ...
        expect(event?.args?.payAmount).to.equal(
    ...
    it('should be able to be buyable with a TestERC20', async function () {
        ...
        await terc20
            .connect(alice)
            .approve(scInstance.address, ethers.utils.parseUnits('500', 18))
        await expect(
            scInstance
                .connect(alice)
                .buy(
            ...
            .to.be.revertedWithCustomError(
                scInstance,
                'NamefiServiceCredit_PayAmountInsufficient'
            )
        let tx2 = await scInstance
            .connect(alice)
            .buy(
